...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # Kiểm tra xem premise có kéo theo conclusion hay không
...
def slam(problem, agent) -> Generator:
    ...
    #util.raiseNotDefined()
    # util.raiseNotDefined()
    # add pacman's initial location to KB, add wall info at that location to KB and known_map
    KB.append(PropSymbolExpr(pacman_str, pac_x_0, pac_y_0, time=0))
    KB.append(~PropSymbolExpr(wall_str, pac_x_0, pac_y_0))
    known_map[pac_x_0][pac_y_0] = 0

    for t in range(agent.num_timesteps):
        # add pacphysics, aciton, percept info to KB 
        ...
        for coord in non_outer_wall_coords:
            ...
            if (entails(conjoin(KB), wall) and entails(conjoin(KB), ~wall)):
                print("BIG PROBLEM WITH KB")
                util.raiseNotDefined()

            if (entails(conjoin(KB), wall) and not entails(conjoin(KB), ~wall)):
                KB.append(wall)
                known_map[x][y] = 1
            elif (entails(conjoin(KB), ~wall) and not entails(conjoin(KB), wall)):
                KB.append(~wall)
                known_map[x][y] = 0
        ...
        for coord in non_outer_wall_coords:
            ...
            # sanity check 
            if (entails(conjoin(KB), pacman_t) and entails(conjoin(KB), ~pacman_t)):
                print("BIG PROBLEM WITH KB")
                util.raiseNotDefined()

            if findModel(conjoin(KB + [pacman_t])):
                possible_locations.append(coord)
            if (entails(conjoin(KB), pacman_t) and not entails(conjoin(KB), ~pacman_t)):
                KB.append(pacman_t)
            elif (entails(conjoin(KB), ~pacman_t) and not entails(conjoin(KB), pacman_t)):
                KB.append(~pacman_t)

                # call agent.moveToNextState(...) 
        agent.moveToNextState(agent.actions[t])
        "*** END YOUR CODE HERE ***"
